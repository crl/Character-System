    ...
    public class ClimbCrouch : MotionControllerMotion
    {
        ...
        public override void Update(float rDeltaTime, int rUpdateIndex)
        {
            ...
            if (!mHasArrived)
            {
                ...
                if (!mHasArrived && lDistance < 0.03f)
                {
                    mHasArrived = true;
                }
            }
        ...
        public virtual bool TestGrab(Vector3 rPosition, Quaternion rRotation, Vector3 rForward, float rRadius, float rBottom, float rTop)
        {
            ...
            if (ShowDebug)
            {
                GraphicsManager.DrawLine(rPosition + (mActorController._Transform.up * rTop), rPosition + (mActorController._Transform.up * rTop) + (rForward * -_BodyTargetOffset.z * 1.5f), Color.green, null, 2f);
            }
#endif
