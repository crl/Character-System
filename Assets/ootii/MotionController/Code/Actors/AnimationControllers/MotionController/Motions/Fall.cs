...
namespace com.ootii.Actors.AnimationControllers
{
    ...
    public class Fall : Jump
    {
        ...
        public override bool TestActivate()
        {
            ...
            // by our velocity. So, use that to determine if we're falling
            if (mActorController.UseTransformPosition)
            {
                if (!mActorController.IsGrounded && !IsInMotionState)
                {
                    if (mMotionLayer.ActiveMotion == null ||
                        mMotionLayer.ActiveMotion.Category != EnumMotionCategories.DEATH)
                    {
                        return true;
                    }
                }
        ...
        public override bool OnInspectorGUI()
        {
            ...
            if (EditorHelper.FloatField("Max Fall Time", "Maximum amount of time we'll remain in the jump or fall.", MaxJumpTime, mMotionController))
            {
                lIsDirty = true;
