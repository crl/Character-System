...
{
    /// <summary>
    /// Flying motion that allows for 360 degrees of movement.
    /// </summary>
    ...
    public class KnockedDown : MotionControllerMotion
    {
        ...
        /// <summary>
        /// Determines if we wake when attacked
        /// </summary>
        public bool _WakeOnAttacked = true;
        public bool WakeOnAttacked
        {
            get { return _WakeOnAttacked; }
            set { _WakeOnAttacked = value; }
        }

        // Flag used to wake the sleeper
        protected bool mWake = false;

        // Stores the last state so we can return
            ...
            : base()
        {
            _Priority = 50;
            _ActionAlias = "";
            _Category = EnumMotionCategories.EFFECT;
        ...
        public override bool TestUpdate()
        {
            ...
            // Exit once we're in the idle pose
        ...
        public override bool Activate(MotionControllerMotion rPrevMotion)
        {
            ...
            // Store the stance and change it
            mStoredStance = mActorController.State.Stance;
            mActorController.State.Stance = EnumControllerStance.UNCONCIOUS;
        ...
        /// <summary>
        /// Function used to wake the sleeping actor
        /// </summary>
        public void Wake()
        {
            mWake = true;
        ...
        /// <param name="rMovementDelta">Root-motion linear velocity relative to the actor's forward</param>
        ...
        {
            // Test if we're supposed to go to wake up
            if (mMotionLayer._AnimatorStateID == STATE_KnockedDown && mMotionLayer._AnimatorTransitionID == 0)
            {
                if (_ActionAlias.Length > 0 && mMotionController._InputSource != null)
                {
                    if (mMotionController._InputSource.IsJustPressed(_ActionAlias))
                ...
                {
                    mWake = false;
                    mMotionController.SetAnimatorMotionPhase(mMotionLayer._AnimatorLayerIndex, PHASE_WAKE, true);
                }
            }
        }
        ...
        public override void OnMessageReceived(IMessage rMessage)
        {
            ...
            if (!mIsActive)
            {
                ...
                if (rMessage is Navigation.NavigationMessage)
                {
                    ...
                    if (lMessage != null && lMessage.ID == Navigation.NavigationMessage.MSG_NAVIGATE_KNOCKED_DOWN)
                    {
                        if (!mIsActive)
                        {
            ...
            // Wake if we're attacked or take damage
            else if (WakeOnAttacked)
            {
                if (rMessage is CombatMessage)
                {
                    CombatMessage lMessage = rMessage as CombatMessage;
                    if (lMessage.Defender == mMotionController.gameObject)
                    {
        ...
        public override bool OnInspectorGUI()
        {
            ...
            if (EditorHelper.TextField("Action Alias", "Action alias that starts the action and then exits the action.", ActionAlias, mMotionController))
            {
                lIsDirty = true;
                ActionAlias = EditorHelper.FieldStringValue;
            }

            if (EditorHelper.BoolField("Wake on Attack", "Determines the character wakes when attacked.", WakeOnAttacked, mMotionController))
            {
                lIsDirty = true;
                WakeOnAttacked = EditorHelper.FieldBoolValue;
            }
