    ...
    public class Damaged : MotionControllerMotion
    {
        ...
        public override bool TestUpdate()
        {
            ...
            if (mMotionLayer._AnimatorStateID == STATE_IdlePose)
            {
                ...
                {
                    // If we're re-entering, do exit... stay in the motion
                    if (mMotionController.State.AnimatorStates[mMotionLayer._AnimatorLayerIndex].MotionPhase != PHASE_START)
                    {
                        return false;
                    }
                }
        ...
        public override void Update(float rDeltaTime, int rUpdateIndex)
        {
            ...
            if (mMotionLayer._AnimatorTransitionID == TRANS_EntryState_Damaged_0 ||
                mMotionLayer._AnimatorTransitionID == TRANS_EntryState_Death_0 ||
                mMotionLayer._AnimatorTransitionID == TRANS_EntryState_Death_180)
            {
            ...
            if (mMotionLayer._AnimatorStateID == STATE_Damaged_0)
            {
                if (mActorController.State.Stance == EnumControllerStance.COMBAT_MELEE_SWORD_SHIELD)
                {
            ...
            if (rMessage is CombatMessage)
            {
                ...
                else if (lCombatMessage.Defender == mMotionController.gameObject)
                {
                    if (rMessage.ID == CombatMessage.MSG_DEFENDER_DAMAGED)
                    {
                        ...
                        {
                            mMotionController.SetAnimatorMotionPhase(mMotionLayer._AnimatorLayerIndex, PHASE_START, Parameter, false);
                        }
