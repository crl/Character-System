...
public class NavMeshInputSourceEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        bool lNewUseStrafingInput = EditorGUILayout.Toggle(new GUIContent("Use Strafe Mode", "Determines if we simulate input as if we're strafing."), mTarget.UseStrafingInput);
        if (lNewUseStrafingInput != mTarget.UseStrafingInput)
        {
            mIsDirty = true;
            mTarget.UseStrafingInput = lNewUseStrafingInput;
        }
        ...
        float lNewNormalizedSpeed = EditorGUILayout.FloatField(new GUIContent("Normalized Speed", "Normalized speed when not in the slow range (0 = idle, 0.5 = walk, 1.0 = run)."), mTarget.NormalizedSpeed);
        if (lNewNormalizedSpeed != mTarget.NormalizedSpeed)
        {
            mIsDirty = true;
            mTarget.NormalizedSpeed = lNewNormalizedSpeed;
        }
        ...
        EditorGUILayout.LabelField(new GUIContent("Slow Speed", "Normalized speed to use when within the slow distance"), GUILayout.Width(50));
